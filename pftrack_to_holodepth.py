#
# Project: HoloDepth - PFTrack Exporter (Fixed for Desktop Export)
# Version: 3.1
# Python 2 Compatible
#
import pfpy
...
def export_for_holodepth(scene_name, image_extension="jpg", output_filename="models.json"):
    print "--- Starting HoloDepth Exporter V3.1 ---"

    try:
        cam = pfpy.getCameraRef(0)
    except:
        print "Error: Could not get PFTrack camera."
        return
        ...
        try:
            camera_info = {
                ...
                "orientation": rotation_matrix_to_axis_angle(
                    euler_to_rotation_matrix(cam.getEulerRotation(frame, 'xyz'))
                ),
            ...
            print "Warning: Skipping frame %d due to error: %s" % (frame, str(e))
            continue

    # ✅ Flattened output (no extra list wrapper)
    final_output = camera_data_list

    # ✅ Always export to Desktop
...
# ---------- Run Export ----------
export_for_holodepth("SceneName")
